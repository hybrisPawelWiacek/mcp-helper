...
export default {
  ...
  async execute(context) {
    ...
    try {
      ...
      const minimumServers = await this.checkMinimumServers(serverCards);
   ...
   */
  async checkMinimumServers(serverCards) {
    try {
      // Load minimum servers configuration
      const minimumServers = await serverCards.getMinimumServers();
      const required = ['serena', 'sequentialthinking', 'context7'];
      
      // Check current configuration
      const config = await serverCards.configManager?.loadConfig();
